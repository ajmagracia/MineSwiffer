...
class Square2 extends Component {
  ...
  componentDidUpdate(prevProps) {
    let { playing, bomb, lastContent } = this.props;
    if (
      playing === false &&
      prevProps.playing === true &&
      lastContent === 'B'
    ) {
      if (bomb === 'B') {
        console.log('click');
  ...
  createSquareHTML = () => {
    let { clickStatus, rightClicked, rightClickCounter } = this.state;
    let { bomb } = this.props;
    let content;
    let innerDiv;
    ...
    if (clickStatus === 'clicked') {
      content = bomb;
      if (bomb === 'B') {
        innerDiv = <div className="burst-8" />;
  ...
  handleRightClick = (e) => {
    e.preventDefault();
    let { clickStatus, rightClickCounter } = this.state;
